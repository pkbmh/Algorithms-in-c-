...
public:
	...
	void insert(vector <int> x , vector <int> y, bool clock)
	{
		point p, q, r;
        int val, val2, i;
        val = x[0], val2 = y[0];
        ...
        root->next = NULL;
        root->prev = NULL;
		vector < pair < int, int > > circular; 
        root->data = val;
        root->data2 = val2;
        //circular.push_back(make_pair(x[0], y[0]));
        for (i = 1; i < x.size(); i++ ) {
        /*	if (i > 1) {
        		p.x = circular[circular.size() - 2].first; p.y = circular[circular.size() - 2].second;
        		q.x = circular[circular.size() - 1].first; q.y = circular[circular.size() - 1].second;
        		r.x = x[i]; r.y = y[i];
        		if ((orientation(p, q, r) != 2 or orientation(p, q, r) != 0) and !clock) {
        			continue;
        		}
        		if ((orientation(p, q, r) != 1 or orientation(p, q, r) != 0) and clock) {
        			continue;
        		}		
        	}
	    circular.push_back(make_pair(x[i], y[i]));*/
...
class convex_hull
{
	...
	{
        struct hull result;
        //cout << xh[0] << endl; cout << "Mamma mia" << endl;
	//	cout << "My: " << endl;
	...
	struct hull con_hull(vector <int> x, vector <int> y, int start, int end, int num) 
	{
		...
		/*	int cnt = 0;
			int mid = (int) x[start] + ((x[end] - x[start]) >> 1);
    ...
    struct hull merge(hull left, hull right) 
    {
        ...
        struct hull final1;
        bool b1;
        l.insert(left.x_cord, left.y_cord, false);
        r.insert(right.x_cord, right.y_cord, true);
        //l.print1();
        //r.print1();
       // printf("left\n");
    	...
    	while (tempr1 != n4) {
			...
			tempr1 = tempr1->next;           //Change here
    	}
    	...
    	while(tempr2 != n1) {
    		...
    		tempr2 = tempr2->next;                   //Change here
    	}
...
int main()
{
    ...
    for (i = 0; i < n; i++ ) {
        ...
        //xh.push_back(tmpx);
        //yh.push_back(tmpy);
        cord.push_back(make_pair(tmpx, tmpy));
        p[i].x = tmpx;
        p[i].y = tmpy;
    }
    ...
    for (int i = 0; i < cord.size(); i++) {
		xh.push_back(cord[i].first);
        yh.push_back(cord[i].second);
    }
    convex_hull h1;
    //h1.sort_xcord(xh, yh);
    //h1.print(xh, yh);
    h1.run();
    //cout << "Morph code: " << endl;
    //convexHull(p, n);
